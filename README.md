...
- AgentTeams v1.2.0 的 1 个 Manager + 4 个专业 Worker 配置清单。

AgentTeams 的本机运行尚未启用：官方本地模式需要把宿主 Docker Socket 挂载给
Manager，拥有较高容器管理权限。启用前必须由使用者明确选择高权限官方模式，或采用
更安全但集成工作更多的无 Socket 方案。仓库内现有 Runner 与 Worker 权限边界不依赖
该决定，Agent 也没有补丁审批接口。
...
参赛材料位于 `materials/` 与 `output/pdf/`：当前包含 14 页初赛方案 PDF、3～5 分钟
演示脚本和提交前检查清单。PDF 中明确标注 AgentTeams 真实运行证据仍待安全授权后补齐。
