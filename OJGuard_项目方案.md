...
### 8 月 2～3 日：范围冻结与 AgentTeams 基线

- 建立代码仓库结构和开源许可证；
- 固定 AgentTeams v1.2.0；
- 跑通 Manager、两个 Worker、Matrix 和共享文件；
- 定义题包格式、状态机和 Pydantic Schema；
- 完成五个 Agent Identity；
- 创建主 Demo 题包。

**验收：** Manager 能拆分任务，Worker 能在共享目录写入结构化结果并在 Matrix 汇报。
...
- C++17 编译与运行；
- CPU、内存、时间、进程和输出限制；
- Validator、Oracle 和候选程序执行；
- 基础差分测试；
- 证据固化和重放；
- 独立 Runner API。

**验收：** 不依赖 LLM 即可稳定复现整数溢出和错误程序反例。

### 8 月 7～9 日：Agent、Skill 与协同

- 完成四个 Worker 的身份、任务模板和权限；
- 实现规格提取、程序审计、测试生成、差分测试和 Checker 审计 Skill；
- 接入 Runner 工具；
- 实现共享 TaskContext、Finding 和 Evidence；
- 展示冲突任务和补充验证。

**验收：** 四类主 Demo 缺陷均经过 Agent 协作和真实执行生成证据。
...
- 修改题包副本并保存补丁；
- 重新执行完整检查；
- 增加失败、超时或 Oracle 冲突路径；
- 生成 HTML 审计报告。

**验收：** 跑通 `BLOCKED → 第一次审批 → 修复副本 → 回归 → READY_FOR_RELEASE 建议 → 第二次确认`。
...
**验收：** 报告中的每个数字均可由脚本重新计算。

### 8 月 13～14 日：完整 Vue 前端

- 实现九个确认页面及路由、状态管理和主题；
- 接入上传、任务、Finding、Evidence、测试、补丁、回归和基准 API；
- 接入 SSE/WebSocket Agent 时间线；
- 增加 Element 原始轨迹入口；
- 在设置页展示 DeepSeek 预算、RAG 预留状态和系统健康信息；
- 完成加载、空状态、失败、超时和权限拒绝反馈。

**验收：** 完整 Demo 可主要通过 OJGuard 前端完成，且所有 Agent 状态和证据均来自真实后端。
...
项目将规格提取、程序审计、差分测试、Checker 审计和发布门禁沉淀为可复用 Skill，通过共享状态与 Trace 保存证据。初赛 Demo 将真实发现四类预埋缺陷，展示异常处理、人工审批和修复前后对比，并用原创基准题包报告缺陷召回率、误阻断率和证据重放率。目前已完成方案、接口与评测设计，核心 Agent、Skill、Runner、适配器和基准集将开源。
...
本表区分“方案已经覆盖”和“工程证据已经完成”。初赛可以以设计为主，但复赛要求可执行 AgentTeams 代码和 Demo，因此工程未完成项必须按开发计划补齐。

### 21.1 硬性要求

| 赛题要求 | OJGuard 对应内容 | 方案状态 | 提交前证据 |
|---|---|---|---|
| 至少 3 个不同职能 Agent | 1 个 Manager + 4 个专业 Worker | 设计符合 | Agent Identity 清单、Matrix 协作截图 |
| 以 AgentTeams 为协同基点 | Manager–Workers、Matrix、MinIO、Heartbeat、Element | 设计符合，运行待实现 | 固定版本代码、原始房间轨迹、共享任务文件 |
| Agent Identity 清单 | 身份、边界、I/O、Skill、权限、失败与人工介入 | 设计符合 | 按附录 A 导出的正式清单 |
| Skill 必选 | 7 个核心 Skill、统一返回、版本和回滚 | 设计符合，代码待实现 | SKILL.md、Schema、样例调用与测试 |
| MCP 或等价工具契约 | 实际实现本地 OJGuard MCP Server | 设计符合，代码待实现 | Tool 列表、鉴权、调用 Trace、失败演示 |
| 至少两项上下文能力 | 共享状态管理 + 轨迹可观测 | 设计符合，代码待实现 | SQLite 状态、共享文件、JSONL Trace、前端时间线 |
| 端到端任务闭环 | 上传—拆解—分析—验证—审批—修复—回归—确认—沉淀 | 设计符合，代码待实现 | 主 Demo 视频和完整运行产物 |
| 结果验证 | 编译、Validator、Oracle、差分和 Checker 探测 | 设计符合，代码待实现 | 可复现反例和 replay 结果 |
| 执行证据 | Finding、Evidence、哈希、Trace、报告 | 设计符合，代码待实现 | evidence-index、日志和 HTML 报告 |
| 审批、回滚和审计 | 三级权限、双审批、工作副本和完整回归 | 设计符合，代码待实现 | Diff、审批记录、回归与回滚演示 |
| 经验沉淀 | Finding、反例、修复结果转为回归与基准资产 | 设计符合，代码待实现 | 新增测试、错误模式条目、基准版本 |
...
| 500 字以内简介 | 符合，当前草案少于 500 字 | 根据真实进展更新最后一句 |
| 方案 PPT/PDF | 内容结构已定义 | 生成并进行视觉、事实和敏感信息检查 |
| AgentTeams 代码包 | 初赛可选、复赛必交 | 初赛争取提交最小可执行版本 |
| 样例输入输出和证据 | 已定义 Demo | 运行后替换所有预期描述为真实结果 |
| 开源协议与边界 | Apache-2.0，第三方边界已说明 | 补充依赖许可证清单 |

### 21.3 评分维度审阅

| 评分维度 | 当前方案判断 | 主要补强证据 |
|---|---|---|
| 场景价值与行业可复制性 25% | 符合，主场景已收敛 | 原创基准、真实耗时和误阻断率 |
| 多 Agent 协同与自主闭环 25% | 设计较强 | 原始 Matrix 轨迹、并行任务、冲突和双审批 |
| Skill 工程体系与生态复用 25% | 设计较强 | 真实 SKILL.md、MCP Schema、版本与复用测试 |
| 工程落地、安全与可审计 20% | 方案完整但尚无运行证据 | 一键部署、沙箱、Trace、重放和异常 Demo |
| 开放与开源贡献 5% | 边界清晰 | 可访问仓库、许可证、Demo 和基准数据 |

### 21.4 审阅结论

OJGuard 在方案层面符合 Agent Infra 赛道的硬性要求，没有依赖 RAG、推荐云产品数量或前端效果代替多 Agent 闭环。当前最大的未完成项不是方案缺口，而是 AgentTeams、Skill、MCP、Runner、Vue 前端和评测尚缺真实运行证据。所有宣传材料必须将“计划实现”和“已经验证”严格区分；只有在对应代码、Trace、反例和报告生成后，才能标记为已完成。
