...
OJGuard 符合 Agent Infra 赛题对场景价值、多 Agent 协同、Skill 工程、工程落地与开放复用的要求。提交内容以可运行代码、真实 Java Runner 对照实验、AgentTeams 运行记录、自动化测试和三类固定种子事故报告为依据，不以概念图代替实现。AgentTeams 是事故决策与协作的控制平面，确定性程序只作为 Worker 调用的受控实验、计算、执行和验证工具。
...
| 工程落地与安全可审计（20%） | Vue3 Agent 协同工作台、FastAPI 后端、SQLite 台账、十阶段状态机、审批闸门、幂等批次、关闭核验、JSON/HTML 报告 | AgentRun/Event 事件流、Java Runner 对照证据、事故报告与测试 |
...
- DeepSeek 仅在用户从前端明确启动或恢复 AgentTeams 后调用；页面加载、场景生成、确定性工具执行、测试和证据重放均不产生模型费用。

## 最终检查

- PPT：12 页高信息密度结构，覆盖场景与价值、方案设计、Agent 分工与动态协同、Skill/MCP、上下文与可观测性、安全边界、真实 AgentTeams 验收、可行性/落地/开放计划和 Evidence Index；模板一致性检查通过，未残留示例占位文本、Demo 路线或录制建议。
- PDF：12 页、16:9，全部页面逐页渲染检查通过。
- 前端：事故创建后只包含原始信号，工作台以 Agent 协同为默认页；支持运行前检查、显式启动、SSE 实时事件、人工批准/拒绝和失败恢复；Vue 类型检查通过。
- 后端与工具：Ruff 检查通过；最终完整复跑 66 项自动化测试全部通过，包含主入口、重复启动互斥、人工门禁、多候选路由和失败恢复。
