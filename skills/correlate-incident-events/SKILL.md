...
Dependent tools: `incident.list_signals`, `submission.aggregate_verdicts`, and `deployment.list_changes`. Agent collaboration: the Signal Aggregator produces the normalized timeline; the Root Cause Analyst turns its correlations into competing hypotheses and controlled experiments; the Incident Manager retains open questions when evidence conflicts. Reuse value: the correlation matrix is dimension-driven, so new judge backends and incident types can add dimensions through a Playbook instead of creating a new orchestration flow.
